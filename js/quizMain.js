...
function LeaderBoard(onUse5050, onUseTel, onUsePub) {
    var used5050 = false;
    var usedTel = false;
    var usedPub = false;
...
function QuestionBoard(ocl) {
    ...
    this.right = function(pos) {
        ...
        ansDiv[pos].classList.remove("suggest");
    ...
    this.suggest = function(pos) {
        ansDiv[pos].classList.remove("wrong");
        ansDiv[pos].classList.remove("right");
        ansDiv[pos].classList.add("suggest");
    }
    this.clear = function() {
        for (t = 0; t < 4; t++) {
            ansDiv[t].classList.remove("right");
            ansDiv[t].classList.remove("wrong");
            ansDiv[t].classList.remove("suggest");
        }
    }
...
function Game() {
    ...
    var leaderBoard = new LeaderBoard(function() {
        ...
        parseXML();
        if (clickable) {
            var indice = Math.floor(Math.random() * 100) + 1;
            if (indice <= 60) {
                console.log("montrer bonne réponse");
                var rep = 0;
                while (rep != questionBoard.newestQuestion.rightAnswer) {
                    rep++;
                }
                questionBoard.suggest(rep);
                return true;
            } else if (indice > 60 && indice <= 95) {
                console.log("montrer mauvaise réponse");
                var rep = 0;
                while (rep == questionBoard.newestQuestion.rightAnswer) {
                    rep++;
                }
                ...
                console.log("répond pas");
                window.alert("Votre ami(e) n'a pas répondu. Désolé");
                return true;
            }
        }
    ...
    check = function(i) {
        ...
        if (i == questionBoard.newestQuestion.rightAnswer) {
            ...
            rank++;
            $.get("index.php?page=9", { palier: rank, fini: fini }, console.log('maj réussie'));
            if (rank <= maxrank) {
